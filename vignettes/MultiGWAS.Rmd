---
title: "Run HDL.S across a large number of traits"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Run HDL.S across a large number of traits}
...
convertZr.batch <- function(
  ...
  for(s in seq(1, length(phes), by=step)){
    ...
    names(N) <- colnames(Z)
    ref.data <- HDL.S::HDL.S.reduct.dim(LD.path, z=Z, D=NULL, lam.cut=lam.cut, Dr.path=NULL,
...
create.D <- function(
  ...
  for(ann.name in annot.names){
    ...
    dD <- HDL.S:::normD(dD, LD.path, norm.method = 'none', pattern = pattern)
    D <- cbind(D, dD)
  }
  return(D)
}
...
## convert D to Dr
ref.data <- HDL.S::HDL.S.reduct.dim(
...
## Step 03. Run HDL.S in parallel

```R
run.HDL.S <- function(phe, zr.dir, res.dir, ref.data, overwrite=FALSE){
  ...
  res <- HDL.S::HDL.S.optim(ref.data, N, output.file=tsv.file,
...
for(ann.name in annot.names){
  ...
  ref.data <- HDL.S:::load.Dr(Dr.path, ann.name, lam.cut=lam.cut,
    pattern=pattern, mc.cores=mc.cores, mode=mode)
  tmp <- parallel::mclapply(
    phes, run.HDL.S, zr.dir, res.dir, ref.data, mc.cores=mc.cores
  )
}
```
