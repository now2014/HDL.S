...
When running `HDL.S` (or `HDL`), you may encounter the following warning message:
...
By default, `HDL.S` (or `HDL`) fills GWAS z-scores with zeros when SNPs in the LD reference panel are missing.
...
all.snps <- do.call(rbind, lapply(HDL.S:::list.LD.ref.files(LD.path, suffix='.bim'), read.table, header=FALSE))[, 2]
set.seed(1234)
...
HDL.S.rebuild.ref(LD.path, gwas.snps, LD.path.new, nthreads = 8) # run with 8 CPU cores
```
