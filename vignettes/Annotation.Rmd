...
It marks a significant breakthrough that `HDL.S` integrates an interpretation framework for both binary and continuous genomic annotations.
...
Suppose you have gone through [this tutorial](https://now2014.github.io/HDL.S/articles/Example.html) and 
...
* installed the `HDL.S` and `HDL` packages.
...
* Step 02. Run `HDL.S` on this binary annotation with the birth weight GWAS summary statistics from `HDL`.
...
In the context of continuous annotation, a normalization process should be employed to ensure that the annotation fits with the HDL.S model.
...
D.minmax <- HDL.S:::normD(D, LD.path, norm.method='minmax')
...
Run `HDL.S` with `minmax` normalization.
...
D.scaled <- HDL.S:::normD(D, LD.path, norm.method='scaled')
...
Run `HDL.S` with `scaled` normalization.
...
LD.path <- 'UKB_array_SVD_eigen90_extraction'
D <- readRDS('raw.W.rds')
D.none <- HDL.S:::normD(D, LD.path, norm.method='none')
...
Run `HDL.S` without annotation weights normalization.
