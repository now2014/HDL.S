---
title: "Example of HDL.S"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Example of HDL.S}
...
Here, we illustrate the usage of HDL.S with the **Panel 2. QCed UK Biobank Axiom Array**.
...
saveRDS(gwas1.example, file='gwas.df.rds') # save the gwas.df for HDL.S
```
...
* Step 03. Select & rename **4 required columns** for `HDL.S`:
  ...
  * **N: Sample size**
  ...
  saveRDS(gwas.df, file='gwas.df.rds') # save the gwas.df for HDL.S
  ```
...
* Step 02. Create a binary annotation of all genes.
  ...
  bim <- do.call(rbind, lapply(
    HDL.S:::list.LD.ref.files(LD.path, suffix='.bim'), data.table::fread, header=FALSE
  ))
  bim <- bim[, c(2, 1, 4)]
  ...
  ## create the annotation weights for HDL.S ##
  D <- matrix(1, nrow=length(snps), ncol=1, dimnames=list(snps, 'ENS.gene'))
  
  saveRDS(D, file='D.rds') # save the annotation weights for HDL.S
  ```

**Notice**: `HDL.S` will automatically align `D` to the LD reference panel according to `rsid`, and set the weights of missing variants in the LD reference panel to **ZERO**.
...
t0 <- Sys.time()
res <- HDL.S::HDL.S(D, gwas.df, LD.path, mc.cores=4, stepwise=TRUE, lam.cut=1, mode='memory', Dr.path=NULL)
print(Sys.time() - t0)
```

HDL.S will finish in **~2 mins** with 4 CPU cores.

Down below message in the console during the analysis (birth weight from `HDL`) :

```bash
Starting HDL.S analysis...
307519 out of 307519 (100.00%) SNPs in reference panel are available in the GWAS.
Transfoming z (D) to zr (Dr)...
D is not NULL and Dr.path is not provided. The default path ./HDL.S_Dr will be used.
